...
"""pico_esc.config — BLHeli-S / BlueJay / BlueGill config-block encode/decode.

Field offsets, enum tables, the sine<->BEMF crossover math, value encoders, the 255-byte
config decoder, and the YAML emit/load helpers. Moved verbatim from esctool.py so the wire
bytes and printed strings are byte-identical; esctool.py re-exports these for compatibility.
"""
